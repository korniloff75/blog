...
var Comments = {
	run: function (options) {
		...
		this.ticket_news2 = options.ticket_news2;
	...
	commentDell: function () {
		...
		formData.append("ticket_news2", this.ticket_news2);
		...
		JLoader.request(this.id + "/ajax/addcomment", {
			...
			success: function (response) {
				...
				if (!isNaN(response)) {
					...
					if (response == "Ticket_news2") { Comments.requestReport("Ошибка при проверке безопасности", "red"); }
