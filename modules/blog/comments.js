var Comments = {
	run: function (options) {
		...
		this.ticket_com = options.ticket_com;
	...
	commentDell: function () {
		...
		formData.append("ticket_com", this.ticket_com);
		...
		JLoader.request(this.id + "/ajax/addcomment", {
			...
			success: function (response) {
				...
				if (!isNaN(response)) {
					...
					if (response == "Ticket_com") { Comments.requestReport("Ошибка при проверке безопасности", "red"); }
